...
void meetingValidTime(boookedCalendar firstUser, boookedCalendar secondUser, boookedCalendar *possibleMeetingTime, boookedCalendar firstLimit, boookedCalendar secondLimit)
{
           ...
           endMeet.hour > secondUser.meet[index2].end.hour || endMeet.hour == secondUser.meet[index2].end.hour && endMeet.minute > secondUser.meet[index2].end.minute)
    {
        ...
        if (secondUser.meet[index1].begin.hour == firstUser.meet[index1].end.hour && secondUser.meet[index1].begin.minute == firstUser.meet[index1].end.minute)
            index1++;
        //maybe they could meet before the meeting from the list
        if (index1 == 0 && index2 == 0)
        {
            timeClock auxiliar = minimTime(firstUser.meet[index1].begin, secondUser.meet[index2].begin);
            if (startMeet.hour < auxiliar.hour || startMeet.hour == auxiliar.hour && startMeet.minute < auxiliar.minute)
            {
                possibleMeetingTime->meet[current].begin.hour = startMeet.hour;
                possibleMeetingTime->meet[current].begin.minute = startMeet.minute;
                possibleMeetingTime->meet[current].end.hour = auxiliar.hour;
                possibleMeetingTime->meet[current].end.minute = auxiliar.minute;
                current++;
            }
        }
...
int main()
{
    ...
    //input
    char input1[100] = "[['10:00','10:30'], ['12:00','13:00'], ['16:00','18:45']]";
    char input2[100] = "[['7:00','11:30'], ['12:30','14:30'],['14:30','15:00'], ['16:00','18:30']]";
    char limit1[18] = "['3:00','19:00']";
    char limit2[18] = "['00:00','24:30']";
    int minutes = 15;
    ...
    //logics of proram
    getCalendar(input1, &firstUser);
    getCalendar(input2, &secondUser);
    getCalendar(limit1, &firstLimit);
    getCalendar(limit2, &secondLimit);
